...
@hydra.main(
    config_path="../configs/",
    config_name="TMaze_intrinsic_Experiment.yaml",
    version_base="1.1",
)
def main(cfg: DictConfig) -> None:
    tune.register_env(
        cfg.env.name,
        lambda _: create_env_tmaze(
            {"episode_length": cfg.env.length, "corridor_length": cfg.env.length - 1}
        ),
    )
    ...
    data_path = os.path.join(root_path, cfg.data_dir, f"./TMaze_intrinsic_{time_path}")
